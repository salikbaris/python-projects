wordlelist = ["cigar", "rebut", "sissy", "humph", "awake", "blush", "focal", "evade", "naval", "serve", "heath",
...
while True:
    ...
    for i in x:
        gray.append(i)
    y = input("yellows with \"-\" as placeholders:")
    green = list(input("greens with \"-\" as placeholders:"))
    for i in y:
        if i != "-" and yellows.count(i) != y.count(i):
            yellows.append(i)
    if y != "":
        yellow.append(y)
    ...
    if A:
        for word in wordlelist:
            ...
            if B and gy == 5:
                c = 0
                for i in word:
                    if i in green[c:] and green.index(i, c) == word.index(i, c):
                        gn += 1
                        if i in placeholder_yellows:
                            y += 1
                            placeholder_yellows.remove(i)
                    elif i in placeholder_yellows:
                        y += 1
                        placeholder_yellows.remove(i)
            ...
            for i in green:
                if i != "-":
                    size_g += 1
            if B and gn == size_g and y == size_y:
                suggestions.append(word)
        A = False
    else:
        s_list = suggestions.copy()
        suggestions = []
        for word in s_list:
            placeholder_yellows = []
            for i in yellows:
